...
브라우저 ──HTTPS──▶ Nginx (80/443, 외부 공개)
                     ...
                     └─ /api/*       → 리버스 프록시 → Express (127.0.0.1:4000, 내부 전용)
                                          └─ Google OAuth / Calendar API 호출
```

- Express(4000)는 **외부에 열지 않습니다.** 반드시 Nginx 를 통해서만 접근합니다.
...
보안 그룹: **인바운드 80, 443만 허용.** 4000 포트는 절대 열지 않습니다.
...
|------|------|
| `NODE_ENV` | `production` |
| `HOST` / `PORT` | `127.0.0.1` / `4000` (내부 전용) |
...
curl http://127.0.0.1:4000/api/health           # {"ok":true,...} 확인
```
...
- [ ] `curl http://127.0.0.1:4000/api/health` → `{"ok":true,"googleConfigured":true}`
