...
// ===== AI 호출 래퍼 (서버 전용) =====
// OpenAI 호환 게이트웨이(AWS Bedrock)를 통해 추천 이유/우선순위를 생성한다.
// 실패하면 예외를 던져 호출부(/api/recommend)가 규칙 기반으로 폴백하게 한다.
// AI API 키는 서버에만 존재하며 브라우저로 절대 노출되지 않는다.
...
// 게이트웨이가 IP + 자체 서명 인증서일 수 있어 TLS 검증을 완화한다.
// (사설 게이트웨이 전용. 공개 API 였다면 이렇게 하면 안 됨)
// openai v5+ 는 fetch 기반이라 예전 httpAgent 옵션이 없어졌다.
// 대신 undici Agent 를 dispatcher 로 주입한 custom fetch 를 넘긴다.
